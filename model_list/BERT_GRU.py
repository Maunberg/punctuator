...
class model(full_model):
    def __init__(self, pretrained_model_name="DeepPavlov/rubert-base-cased", 
                 freeze_bert=False, gru_hidden_dim=128, num_gru_layers=1, 
        ...
        self.gru = nn.GRU(768, gru_hidden_dim, num_layers=num_gru_layers, bidirectional=bidirectional, batch_first=True)
        if bidirectional:
            gru_out_dim = gru_hidden_dim * 2
        else:
            gru_out_dim = gru_hidden_dim
    
        self.dropout = nn.Dropout(dropout)
        self.fc1 = nn.Linear(gru_out_dim, num_classes1)
        self.fc2 = nn.Linear(gru_out_dim, num_classes2)
        self.relu = nn.ReLU()
    ...
    def forward(self, input_ids, attention_mask):
        ...
        logits1 = self.fc1(gru_outputs)
        logits2 = self.fc2(gru_outputs)
