...
model_name = 'berts/rubert-base-cased'
model_name = 'berts/bert-base-multilingual-uncased'
way_to_corpus = ['data_raw/tedtalks_en.txt',
                 'data_raw/tanzil_de.txt']
n = 5_000_000
lang = 'ende'
version = 3
lenth_train = 100_000
lenth_valid = 20_000
way_to_norm_text = f'data_norm/{lang}_v{version}.txt'
...
def prepare_data(text, tokenizer=BertTokenizer.from_pretrained(model_name)):
    ...
    for line in tqdm(text):
        ...
        labels_1, tokens = label_input(tokens, labels_1, label=':', label_id=4, mode='punct', labels_count=labels_count_1)
        labels_1, tokens = label_input(tokens, labels_1, label='–', label_id=5, mode='punct', labels_count=labels_count_1)
    ...
    if len(way_to_corpus) == 1:
        ...
        for way_crp in way_to_corpus:
            with open(way_crp, 'r', encoding="utf8") as file:
                lines.extend(file.readlines()[:int(n//len(way_to_corpus))])
